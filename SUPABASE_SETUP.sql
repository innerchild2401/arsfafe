-- =====================================================
-- COMPLETE DATABASE SCHEMA FOR RAG SYSTEM
...
CREATE INDEX IF NOT EXISTS idx_books_text_hash ON books(text_hash);
...
CREATE INDEX IF NOT EXISTS idx_user_book_access_user ON user_book_access(user_id);
CREATE INDEX IF NOT EXISTS idx_user_book_access_book ON user_book_access(book_id);
CREATE INDEX IF NOT EXISTS idx_user_book_access_visible ON user_book_access(user_id, is_visible) WHERE is_visible = true;
CREATE INDEX IF NOT EXISTS idx_user_book_access_owner ON user_book_access(book_id, is_owner) WHERE is_owner = true;
...
CREATE INDEX IF NOT EXISTS idx_parent_chunks_chapter ON parent_chunks(book_id, chapter_title);
...
CREATE INDEX IF NOT EXISTS idx_child_chunks_book ON child_chunks(book_id);
...
CREATE INDEX IF NOT EXISTS idx_child_chunks_embedding ON child_chunks USING hnsw (embedding vector_cosine_ops) WITH (m = 16, ef_construction = 64);
...
CREATE INDEX IF NOT EXISTS idx_chat_messages_book ON chat_messages(book_id);
CREATE INDEX IF NOT EXISTS idx_chat_messages_created ON chat_messages(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_chat_messages_user_book ON chat_messages(user_id, book_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_parsing_corrections_user ON parsing_corrections(user_id);
CREATE INDEX IF NOT EXISTS idx_parsing_corrections_book ON parsing_corrections(book_id);
CREATE INDEX IF NOT EXISTS idx_parsing_corrections_usage ON parsing_corrections(usage_count DESC);
-- HNSW index for vector similarity search (1536 dims fits within 2000 limit)
CREATE INDEX IF NOT EXISTS idx_parsing_corrections_embedding ON parsing_corrections USING hnsw (embedding vector_cosine_ops) WITH (m = 16, ef_construction = 64);
...
-- 2. Run this to set yourself as admin (replace email):
--    UPDATE user_profiles 
--    SET role = 'admin', status = 'approved', has_limits = false
